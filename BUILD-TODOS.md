...
**status:** platform layer shipped — manual gates + production deploy remain  
...
- [x] set up sponsors webhook → `POST /api/webhooks/github-sponsors` (persists to sqlite/d1 + cron fallback)
- [x] sync sponsor tier to user profile after better-auth github login (session hook)
- [x] founding supporter badge logic (first 100 constant + webhook + session sync)
...
- [x] d1 database + schema migrated (`packages/db/drizzle/0000_init.sql`, local libsql via `pnpm db:init`)
- [x] github actions deploy pipeline (`main` → cloudflare, gated by `CLOUDFLARE_DEPLOY_ENABLED`)
- [x] dependabot enabled
- [x] self-hosted umami analytics wired (env-gated script in layout)
...
- [x] configure email + magic link (better-auth plugin + route handler)
...
- [x] build login/signup modals (design system, lowercase, better-auth client)
- [x] username setup on first login (modal + `/api/user/username`)
- [x] session persistence + protected routes (sqlite/d1 via drizzle adapter)
...
- [x] pagefind search (ui component + post-build command)
...
- [x] user lists (create, add, remove — `/dashboard`, profile add-to-list)
